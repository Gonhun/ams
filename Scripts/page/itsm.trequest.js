...
function loadGrid() {
    $.ajax({
        url: urlPath + '/itsm/api/request/get',
        cache: false,
        method: "POST",
        ...
        complete: function (res) {
            table1 = $('#tblRequest').DataTable({
                ...
                "columns": [
                    ...
                    {
                        'data': 'approval_flag',
                        className: 'dt-head-center dt-wrap',
                        render: function (data, type, row) {
                            if (row.it_is_proceess == null && row.hr_is_process == null && row.cc_is_process == null) {
                                return `<span class="badge badge-warning">${row.approval_flag}</span>`
                            }
                            else if (row.it_is_proceess == true && row.hr_is_process == true && row.cc_is_process == true) {
                                return `<span class="badge badge-primary">${row.approval_flag}</span>`
                            }
                            else {
                                return `<span class="badge badge-warning">${row.approval_flag}</span>`
                            }
...
$('#tblRequest tbody').on('click', '#btnDetail', function () {
    ...
    console.log(data);
    window.location = urlPath + "itsm/pengajuan/detail?reqId=" + data.req_id ;
})
...
function loadNrp() {
    if ($("#txtNik").val() == "") {
        ...
        var i = 0;
        $.ajax({
            url: urlPath + '/itsm/api/request/get/emp',
            cache: false,
...
function loadAssetCheck() {
    $.ajax({
        url: urlPath + '/itsm/api/aset/get/byuser',
        cache: false,
        data: JSON.stringify({ nik: $("#txtNik").val() }),
        method: "POST",
        contentType: "application/json; charset=utf-8",
...
function Save() {
    ...
    $.ajax({
        url: urlPath + '/itsm/api/request/insert',
        cache: false,
        method: "POST",
        data: formData,
        //contentType: "application/json; charset=utf-8",
