...
function loadDetail() {
    ...
    $.ajax({
        url: urlPath + 'api/request/get/byid',
        cache: false,
        method: "POST",
        ...
        complete: function (res) {
            ...
            if (_data.req_detail.approval_dept != null) {
                $("#kabagValidation").attr("hidden", false);
                document.getElementById("lblKabagDate").innerHTML = convertJsonDateToShortDate(_data.req_detail.dept_date);
                document.getElementById("lblStatusKabag").innerHTML = _data.dept_flag;
                document.getElementById("remarkKabag").innerHTML = _data.req_detail.dept_remark;
            }

            if (_data.it_is_process != null) {
                $("#itValidation").attr("hidden", false);
                document.getElementById("lblItDate").innerHTML = convertJsonDateToShortDate(_data.it_processed_date);
                document.getElementById("lblStatusIt").innerHTML = _data.it_flag;
                document.getElementById("remarkIt").innerHTML = _data.it_processed_remark;
            }

            if (_data.hr_is_process != null) {
                $("#hrValidation").attr("hidden", false);
                document.getElementById("lblHrDate").innerHTML = convertJsonDateToShortDate(_data.hr_processed_date);
                document.getElementById("lblStatusHr").innerHTML = _data.hr_flag;
                document.getElementById("remarkHr").innerHTML = _data.hr_processed_remark;
            }

            if (_data.cc_is_process != null) {
                $("#ccValidation").attr("hidden", false);
                document.getElementById("lblCcDate").innerHTML = convertJsonDateToShortDate(_data.cc_processed_date);
                document.getElementById("lblStatusCc").innerHTML = _data.cc_flag;
                document.getElementById("remarkCc").innerHTML = _data.cc_processed_remark;
            }
        }
    })
}
